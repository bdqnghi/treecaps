...
class TreeCapsModel():
    ...
    def init_net_treecaps(self):
        ...
        with tf.name_scope('inputs'):
            ...
            shape_of_weight_dynamic_routing_class_caps = [1, self.code_caps_num_caps, self.class_caps_output_dimension * self.class_caps_num_caps, self.num_conv, 1]
            ...
            self.weights["w_t"] = tf.Variable(tf.contrib.layers.xavier_initializer()([self.node_dim, self.output_size]), name='w_t')
            self.weights["w_l"] = tf.Variable(tf.contrib.layers.xavier_initializer()([self.node_dim, self.output_size]), name='w_l')
            self.weights["w_r"] = tf.Variable(tf.contrib.layers.xavier_initializer()([self.node_dim, self.output_size]), name='w_r')
            self.weights["b_conv"] = tf.Variable(tf.zeros([self.output_size,]),name='b_conv')
    ...
    def feed_forward(self):
        with tf.name_scope('network'):  
            ...
            self.code_caps = self.vts_routing(self.primary_variable_caps, self.batch_size, self.output_size, self.num_conv, self.code_caps_num_caps)
    ...
    def vts_routing(self, primary_variable_caps, batch_size, conv_output_size, input_dimension=4, num_output_caps=10):
        """The proposed Variable-to-Static Routing Algorithm."""
        
        num_nodes = tf.shape(primary_variable_caps)[1]
        
        alpha_IJ = tf.zeros((num_nodes * conv_output_size, num_output_caps), dtype=tf.float32)

        primary_variable_caps_transposed = tf.transpose(primary_variable_caps,perm=[0,3,1,2])
        primary_variable_caps_transposed_reshaped = tf.reshape(primary_variable_caps_transposed, shape=[batch_size, input_dimension, -1])

        primary_static_caps, _ = tf.nn.top_k(primary_variable_caps_transposed_reshaped,k=num_output_caps)
        v_J = primary_static_caps
        v_J = tf.transpose(v_J, perm=[0, 2, 1])

        # batch_size, num_nodes x conv_output_size, input_dimension
        u_i = tf.reshape(primary_variable_caps, (batch_size, -1, input_dimension))
    ...
    def conv_node(self, parent_node_embeddings, children_embeddings, children_indices, node_dim):
        ...
        with tf.name_scope('conv_node'):
            w_t, w_l, w_r = self.weights["w_t"], self.weights["w_l"], self.weights["w_r"]
            b_conv = self.weights["b_conv"]
